...
The parser gets all source files under given `bracks` directory as a stream of [vinyl](https://github.com/gulpjs/vinyl) file objects and pipes them [through2](https://github.com/rvagg/through2) transform function, parses them all, pipes the result documents to their destination under `views` directory (in case of [express](http://expressjs.com) application), and calls the `next` middleware on `end` event.
#####Rationale
